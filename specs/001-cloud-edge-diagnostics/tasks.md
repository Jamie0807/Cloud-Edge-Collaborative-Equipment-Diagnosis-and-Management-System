...
**状态：** Draft — 规划清单，不授权创建业务代码、`packages/contracts` 或运行基础设施。
...
所有任务须在下列前置条件满足后才可从 Draft 开始：

1. 用户确认 D-001 至 D-014，特别是通信、认证、文件存储、迁移、`eventId` 幂等与重试策略；将选择回写为批准的 spec/plan，产生长期决定时补 ADR。
...
T-003 和 T-005 可以在 T-002 完成后以 mock 契约并行准备，但真实联调依赖 T-004；T-006 在 T-005 稳定查询 API 后开始。各并行任务必须有不重叠写集。
...
1. 与决策人逐项确认 D-001 至 D-014；没有确认不得用默认值实现。
2. 将已确认协议、技术、存储、认证、文件 URI、时间、幂等/重试、分页和资源指标口径写入规格和计划；对有长期影响的选择记录 ADR。
...
**通过标准：** 所有选择有明确确认记录或仍明确阻断；无隐含技术假设；差异无空白错误。
...
- 创建/修改：`packages/contracts/tests/**`
- 创建/修改：`packages/test-fixtures/**`
- 创建/修改：`apps/*/tests/**`（仅补充端到端夹具/测试）
...
**通过标准：** 报告包含命令、实际结果、通过范围、失败/未运行项和环境限制；不以手工演示替代自动化证据。
...
- 创建/修改：`docs/runbooks/001-local-start-and-demo.md`
- 创建/修改：`docs/verification/001-cloud-edge-diagnostics.md`

**工作与验收：**

1. 记录架构边界、四个 ID 语义、终端→边端→云端→Web 流程、协议、文件路径和不重试/可重试错误。
2. 提供可复制的本地启动、配置、数据库迁移、测试和演示步骤；将所有凭据留在环境变量/示例中，不能填入真实值。
...
**通过标准：** 文档能独立指导启动和演示；追踪关系完整；仅报告实际运行的命令结果。

## 3. 任务边界和当前限制

- 任务 D-001 至 D-014 是决策任务，不能被“先写再改”的代码替代。
- T-002 的 `packages/contracts` 是未来实现写集；本分析 worktree 不创建该目录或任何 schema 文件。
- 只有在 bootstrap 已集成后，`pnpm`、pytest、JUnit、Vitest、Playwright 命令才具有可执行基础。本草案列出的命令是未来验收命令，尚未在当前空 worktree 运行。
