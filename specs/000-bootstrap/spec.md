...
- FR-006：明确原始业务需求文件 `09-ai-fullstack-interview-coding-exercise.md` 是后续业务 spec 输入；本 bootstrap 不实现业务链路。
...
- AC-003：bootstrap 的 spec、plan、tasks 互相引用，并覆盖 AGENTS、pnpm workspace、测试质量、CI、skills、文档、验收和原始业务需求输入。
...
- 后续业务总 spec 如何拆分终端、边端、云端和 Web feature，须以原始业务需求为输入并经用户确认。
